...
public:
    ...
    void * operator new(size_t s, void * stub) {
        ...
        Framework::Element * el= Framework::_cache.search_key(reinterpret_cast<size_t>(stub));
        void * handle;
        if(el) {
            handle = el->object();
            ...
            el = new Framework::Element(handle, reinterpret_cast<size_t>(stub));  // the handled cache is insert-only; object are intentionally never deleted, since they have been created by SETUP!
...
public:
    ...
    int fork(int *main) { return _stub->fork(main); }
