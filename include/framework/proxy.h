...
public:
    ...
    void * operator new(size_t s, void * adapter) {
        ...
        Framework::Element * el= Framework::_cache.search_key(reinterpret_cast<size_t>(adapter));
        void * proxy;
        if(el) {
            proxy = el->object();
            ...
            el = new Framework::Element(proxy, reinterpret_cast<size_t>(adapter));  // the proxied cache is insert-only; object are intentionally never deleted, since they have been created by SETUP!
...
public:
    ...
    int fork(int *main) { return invoke(TASK_FORK, main); }
