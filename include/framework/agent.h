...
public:
    void exec() {
        ...
        if(id().type() < LAST_TYPE_ID) { // in-kernel services
            ...
            db<Framework>(WRN) << "Syscall " << *reinterpret_cast<Message *>(this) << endl;
        }
...
void Agent::handle_task()
{
    ...
    case TASK_ADDRESS_SPACE:
        res = reinterpret_cast<size_t>(task->address_space());
        break;
    case TASK_CODE_SEGMENT:
        res = reinterpret_cast<size_t>(task->code_segment());
        break;
    case TASK_DATA_SEGMENT:
        res = reinterpret_cast<size_t>(task->data_segment());
        break;
    case TASK_CODE:
        res = task->code();
        break;
    case TASK_DATA:
        ...
        break;
    case TASK_MAIN:
        res = reinterpret_cast<size_t>(task->main());
        break;
    case TASK_FORK:
        int (* main)();
        in(main);
        res = reinterpret_cast<unsigned int>(fork(main));
        break;
    default:
        res = UNDEFINED;
    }
