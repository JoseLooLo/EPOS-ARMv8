...
void populate_strings()
{
    ...
    snprintf(string, STRING_SIZE, "0x%016lx", Memory_Map::RAM_BASE);
    set_token_value("RAM_BASE", string);

    snprintf(string, STRING_SIZE, "0x%016lx", Memory_Map::RAM_TOP);
    set_token_value("RAM_TOP", string);

    snprintf(string, STRING_SIZE, "0x%016lx", Memory_Map::RAM_TOP + 1 - Memory_Map::RAM_BASE);
    set_token_value("MEM_SIZE", string);

    snprintf(string, STRING_SIZE, "0x%016lx", (Memory_Map::RAM_TOP + 1 - Memory_Map::RAM_BASE) / 1024);
    set_token_value("MEM_SIZE_KB", string);

    snprintf(string, STRING_SIZE, "0x%016lx", Memory_Map::MIO_BASE);
    set_token_value("MIO_BASE", string);

    snprintf(string, STRING_SIZE, "0x%016lx", Memory_Map::MIO_TOP);
    set_token_value("MIO_TOP", string);

    snprintf(string, STRING_SIZE, "0x%016lx", Memory_Map::MIO_TOP + 1 - Memory_Map::MIO_BASE);
    set_token_value("MIO_SIZE", string);

    snprintf(string, STRING_SIZE, "0x%016lx", (Memory_Map::MIO_TOP + 1 - Memory_Map::MIO_BASE) / 1024);
    set_token_value("MIO_SIZE_KB", string);

    if(Memory_Map::BOOT_STACK != Memory_Map::NOT_USED)
        snprintf(string, STRING_SIZE, "0x%016lx", Memory_Map::BOOT_STACK);
    else
        string[0] = '\0';
    set_token_value("BOOT_STACK", string);

    if(Memory_Map::BOOT != Memory_Map::NOT_USED)
        snprintf(string, STRING_SIZE, "0x%016lx", Memory_Map::BOOT);
    else
        string[0] = '\0';
    set_token_value("BOOT", string);

    if(Memory_Map::IMAGE != Memory_Map::NOT_USED)
        snprintf(string, STRING_SIZE, "0x%016lx", Memory_Map::IMAGE);
    else
        string[0] = '\0';
    set_token_value("IMAGE", string);

    if(Memory_Map::SETUP != Memory_Map::NOT_USED)
        snprintf(string, STRING_SIZE, "0x%016lx", Memory_Map::SETUP);
    else
        string[0] = '\0';
    set_token_value("SETUP", string);

    if(Memory_Map::INIT != Memory_Map::NOT_USED)
        snprintf(string, STRING_SIZE, "0x%016lx", Memory_Map::INIT);
    else
        string[0] = '\0';
    set_token_value("INIT", string);

    if(Memory_Map::APP_CODE != Memory_Map::NOT_USED)
        snprintf(string, STRING_SIZE, "0x%016lx", Memory_Map::APP_CODE);
    else
        string[0] = '\0';
    set_token_value("APP_CODE", string);

    if(Memory_Map::APP_DATA != Memory_Map::NOT_USED)
        snprintf(string, STRING_SIZE, "0x%016lx", Memory_Map::APP_DATA);
    else
        string[0] = '\0';
    set_token_value("APP_DATA", string);

    if(Memory_Map::SYS_CODE != Memory_Map::NOT_USED)
        snprintf(string, STRING_SIZE, "0x%016lx", Memory_Map::SYS_CODE);
    else
        string[0] = '\0';
    set_token_value("SYS_CODE", string);

    if(Memory_Map::SYS_DATA != Memory_Map::NOT_USED)
        snprintf(string, STRING_SIZE, "0x%016lx", Memory_Map::SYS_DATA);
    else
        string[0] = '\0';
    set_token_value("SYS_DATA", string);

    if(Memory_Map::SYS_STACK != Memory_Map::NOT_USED)
        snprintf(string, STRING_SIZE, "0x%016lx", Memory_Map::SYS_STACK);
    else
        string[0] = '\0';
    set_token_value("SYS_STACK", string);

    if(Memory_Map::SYS_HEAP != Memory_Map::NOT_USED)
        snprintf(string, STRING_SIZE, "0x%016lx", Memory_Map::SYS_HEAP);
    else
        string[0] = '\0';
